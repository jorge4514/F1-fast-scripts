...
# cargar la sesion
session = ff1.get_session(2023, 'Miami', 'FP2')
laps = session.load_laps(with_telemetry=True)

driver_1, driver_2, driver_3, driver_4 = 'ALO', 'HAM', 'VER','LEC'
...
fastest_driver_4 = session.laps.pick_driver(driver_4).pick_fastest()
...
telemetry_driver_4 = fastest_driver_4.get_telemetry()
...
telemetry_driver_4['Driver'] = driver_4

telemetry = pd.concat([telemetry_driver_1, telemetry_driver_2, telemetry_driver_3, telemetry_driver_4])
num_minisectors = 25
...
telemetry.loc[telemetry['Fastest_driver'] == driver_4,
'Fastest_driver_int'] = 4
...
cmap = ListedColormap([color_1, color_2, color_3, color_4])
...
plt.title(f'Lap Comparison between {driver_1} and {driver_2} and {driver_3} and {driver_4}')
plt.gca().add_collection(lc_comp)
plot_title = f"{session.event.year} {session.event.EventName} - {session.name} - {driver_1} VS {driver_2} VS {driver_3} VS {driver_4}"
