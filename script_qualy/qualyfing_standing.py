...
fastf1.Cache.enable_cache(r'C:\Users\jorge\Desktop\fastf1\cache') # optional  # replace with your cache directory

# we only want support for timedelta plotting in this example
fastf1.plotting.setup_mpl(mpl_timedelta_support=True, color_scheme=None, misc_mpl_mods=False)

session = fastf1.get_session(2023, 'Baku Grand Prix', 'Q')
session.load()
...
ax.set_yticklabels(fastest_laps['Driver'] + '  +'  + (fastest_laps['LapTimeDelta'].dt.total_seconds() % 60).apply(lambda x: '{:.3f}'.format(x)).astype(str))
